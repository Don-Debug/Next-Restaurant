      ...
      <div className="galery-bg my-[5rem] flex flex-col justify-center items-center w-full h-auto ">
        ...
        <div className="flex flex-row flex-wrap justify-center items-start lg:px-[5rem] md:px-[2rem] px-[1rem] py-[3rem] md:gap-10 gap-6">
          {galery.slice(0, numDisplayed).map((post, index) => (
            <div
              key={index}
              className="flex justify-center relative shadow-2xl md:w-[20rem] sm:w-[15rem] w-[10rem] h-auto overflow-hidden"
            >
              <Image
                src={post.image}
                alt={`${index}`}
                width={1000}
                height={1000}
                className=" w-[100%] md:h-[20rem] sm:h-[15rem] h-[10rem] object-cover pointer-events-none rounded-md"
              />
              <button
        ...
        {!isExpanded && numDisplayed < galery.length && (
          <button
            className=" w-fit mx-auto my-[5rem] text-[1.2rem] px-[2rem] py-3 rounded-[1.5rem] bg-[#f1f1f7] shadow-2xl border-x-[5px] border-[#1f1f1d]  text-[#1f1f1d] font-semibold"
